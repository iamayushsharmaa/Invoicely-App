...
import 'package:invoice/features/auth/data/models/google_signin_request.dart';
import 'package:invoice/features/auth/data/models/sign_in_request.dart';
import 'package:invoice/features/auth/data/models/signup_request.dart';
...
class AuthService {
  ...
  Future<AuthenticationResponse> continueWithGoogle(GoogleSignInRequest request) async {
    try {
      final response = await dio.post('/api/v1/auth/google', data: request.toJson());
      return AuthenticationResponse.fromJson(response.data);
    } catch (e) {
      if(e is DioException && e.response != null){
        return AuthenticationResponse.fromJson(e.response!.data);
